...
exports.createSearch = function(key,opts,cb){
  ...
  exports.client.send_command('FT.INFO',[key],function(err,info) {
    if (err) { 
      ...
      // but only if the opts.define option is set with value [true].
      // We don't want read attempts on non existent indexes (that do not include the correct schema) to be able to create
      // the index without the appropriate schema. Once our "index" function below attempts to write some data to this index,
      // we should be good to go.
      if (String(err).indexOf('Unknown Index name') > 0 && opts.define && opts.define === true){
        let args = [
          key,
          'SCHEMA'
        ];
        ...
        if(!opts.schema){
          // If no custom schema is passed, default back to including a "payload" field in the schema.
          args.push('payload','TEXT');
        } else {
          Object.keys(opts.schema).forEach(function(key){
            args.push(key.toString(), opts.schema[key].toString());
          });
        }
...
Query.prototype.end = function(fn){
  ...
  if(!this.search.opts.noContent || (this.search.opts.noContent && this.search.opts.noContent === true)){
    // Default to not show content (which refers to field:value pairs)
    args.push('NOCONTENT');
  }
  if(this.search.opts.noStopWords && this.search.opts.noStopWords === true){
    args.push("NOSTOPWORDS");
  }
  if(this.search.opts.withPayloads){
...
Search.prototype.index = function(str, id, fields, fn){
  // 'fields' is an optional parameter
  if(typeof fields === "function" && (typeof fn === 'undefined' || fn === null)){
    var fn = fields;
    fields = null;
  }
  ...
  if(opts.withPayloads && opts.withPayloads === true){
    args.push('PAYLOAD', (typeof str === 'object' ? JSON.stringify(str) : str.toString()));
  }
  ...
  if(typeof fields === 'object' && Object.keys(fields).length > 0){
    ...
    Object.keys(fields).forEach(function(key){
      ...
      var str2 = fields[key];
      args.push((typeof str2 === 'object' ? JSON.stringify(str2) : str2.toString()));
    });
  } else {
    // Set a default 'payload' field, otherwise there's no fields available to match the search against...
    args.push('FIELDS','payload',(typeof str2 === 'object' ? JSON.stringify(str2) : str2.toString()));
  }
